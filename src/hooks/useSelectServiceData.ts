...
export function useSelectServiceData({
  ...
  const [data, setData] = useState<SelectServiceData>({
    ...
    fullName:
      initialData.fullName || bookingFormData?.personalInfo?.fullName || "",
  ...
  const calculateTotals = () => {
    const selectedAddOnServices = addOns.filter((addon) =>
      data.addOns.includes(addon.id)
    );
    const addOnsTotal = selectedAddOnServices.reduce(
      (sum, addon) => sum + addon.price,
      0
    );
    const servicesTotal = data.selectedServices.reduce(
  ...
  const validateForm = () => {
    ...
    const missingFields = requiredFields.filter(
      (field) => !data[field as keyof SelectServiceData]
    );
