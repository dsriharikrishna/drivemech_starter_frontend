...
export default function CreateMpinPage(): React.ReactNode {
  ...
  const methods = useForm({
    resolver: zodResolver(createMpinSchema),
    defaultValues: {
      mpin: "",
      confirmMpin: "",
    },
  ...
  const handleMpinChange = (
    ...
    if (!value) {
      ...
      setter(copy);
      // Update form value
      const fieldName = type === "new" ? "mpin" : "confirmMpin";
      setValue(fieldName, copy.join(""), { shouldValidate: true });
      return;
    }
  ...
  const handlePaste = (
    ...
    if (type === "new") {
      setNewMpin(arr);
      setValue("mpin", arr.join(""), { shouldValidate: true });
    } else {
      setConfirmMpin(arr);
      setValue("confirmMpin", arr.join(""), { shouldValidate: true });
    }
  ...
  const onSubmit = async (data: { mpin: string; confirmMpin: string }) => {
    setError(null);
    console.log(data);

    // TODO: Integrate Redux thunks
    // Dispatch createMpin thunk
    // const result = await dispatch(createMpin({
    //   mpin: data.mpin,
    ...
    router.push(`${authPrefix}/confirm`);
    // } else {
    //   setError(apiError || "Failed to create MPIN. Please try again.");
    // }
  };

  const inputClass =
    "w-14 h-14 text-center text-2xl font-semibold rounded-xl border border-gray-300 bg-gray-50 shadow-sm transition-all";
