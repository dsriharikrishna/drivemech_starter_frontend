...
>(
  ...
  async (notificationId, { rejectWithValue }) => {
    try {
      const endpoint = API_CONFIG.ENDPOINTS.GET_NOTIFICATION_BY_ID.replace(
        ":id",
        notificationId
      );
...
>("notification/markAsRead", async (notificationId, { rejectWithValue }) => {
  try {
    const endpoint = API_CONFIG.ENDPOINTS.MARK_AS_READ.replace(
      ":id",
      notificationId
    );
...
>("notification/markAllAsRead", async (_, { rejectWithValue }) => {
  try {
    const response = await apiService.post(
      API_CONFIG.ENDPOINTS.MARK_ALL_AS_READ
    );
    return response.data.data || response.data;
  } catch (error: any) {
    return rejectWithValue(
...
>(
  ...
  async (notificationId, { rejectWithValue }) => {
    try {
      const endpoint = API_CONFIG.ENDPOINTS.DELETE_NOTIFICATION.replace(
        ":id",
        notificationId
      );
...
>("notification/deleteAllNotifications", async (_, { rejectWithValue }) => {
  try {
    const response = await apiService.delete(
      API_CONFIG.ENDPOINTS.DELETE_ALL_NOTIFICATIONS
    );
    return response.data.data || response.data;
  } catch (error: any) {
    return rejectWithValue(
...
>("notification/getUnreadCount", async (_, { rejectWithValue }) => {
  try {
    const response = await apiService.get(
      API_CONFIG.ENDPOINTS.GET_UNREAD_COUNT
    );
    return response.data.data || response.data;
  } catch (error: any) {
    return rejectWithValue(
      ...
      .addCase(markAsRead.fulfilled, (state, action) => {
        ...
        if (index !== -1) {
          ...
          if (wasUnread) {
            ...
            const type = action.payload.type;
            if (state.unreadByType[type]) {
              state.unreadByType[type] = Math.max(
                0,
                state.unreadByType[type]! - 1
              );
            }
          }
        }
