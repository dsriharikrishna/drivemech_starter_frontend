...
const responsiveStyles: Record<Variant, string> = {
  ...
  blockquote:
    "text-sm xs:text-base sm:text-lg md:text-xl lg:text-2xl italic border-l-4 pl-4 border-gray-300 dark:border-gray-600",
  // ✅ NEW VARIANTS
...
const fixedStyles: Record<Variant, string> = {
  ...
  overline: "text-xs xs:text-xs sm:text-sm uppercase tracking-wide",
  blockquote:
    "text-base xs:text-lg sm:text-xl md:text-2xl italic border-l-4 pl-4 border-gray-300 dark:border-gray-600",
          ...
          ? {
              fontSize: `clamp(${minFontSize || fluidTypographyScales[variant]?.min || "1rem"}, 
                 ...
                 (${
                   parseFloat(
                     maxFontSize ||
                       fluidTypographyScales[variant]?.max ||
                       "1.25rem"
                   ) -
                   parseFloat(
                     minFontSize ||
                       fluidTypographyScales[variant]?.min ||
                       "1rem"
                   )
                 }) * 
...
export function Link(
  props: Omit<TypographyProps, "variant" | "as"> & {
    variant?: "body" | "caption" | "overline" | "small";
  }
) {
  return (
    <Typography variant={props.variant || "body"} color="primary" {...props} />
  );
}
