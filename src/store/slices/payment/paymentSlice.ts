...
>("payment/getPaymentMethods", async (_, { rejectWithValue }) => {
  try {
    const response = await apiService.get(
      API_CONFIG.ENDPOINTS.GET_PAYMENT_METHODS
    );
...
>("payment/addPaymentMethod", async (payload, { rejectWithValue }) => {
  try {
    const response = await apiService.post(
      API_CONFIG.ENDPOINTS.ADD_PAYMENT_METHOD,
      payload
    );
    return response.data.data || response.data;
  } catch (error: any) {
...
>("payment/removePaymentMethod", async (methodId, { rejectWithValue }) => {
  try {
    const endpoint = API_CONFIG.ENDPOINTS.REMOVE_PAYMENT_METHOD.replace(
      ":id",
      methodId
    );
...
>("payment/setDefaultPaymentMethod", async (methodId, { rejectWithValue }) => {
  try {
    const endpoint = API_CONFIG.ENDPOINTS.SET_DEFAULT_PAYMENT_METHOD.replace(
      ":id",
      methodId
    );
...
>("payment/verifyPayment", async (payload, { rejectWithValue }) => {
  try {
    const endpoint = API_CONFIG.ENDPOINTS.VERIFY_PAYMENT.replace(
      ":id",
      payload.paymentId
    );
    const response = await apiService.post(endpoint, {
...
>("payment/refundPayment", async (payload, { rejectWithValue }) => {
  try {
    const endpoint = API_CONFIG.ENDPOINTS.REFUND_PAYMENT.replace(
      ":id",
      payload.paymentId
    );
    const response = await apiService.post(endpoint, {
...
>("payment/getTransactions", async (filters, { rejectWithValue }) => {
  try {
    const response = await apiService.get(
      API_CONFIG.ENDPOINTS.GET_TRANSACTIONS,
      {
        params: filters,
      }
    );
...
>("payment/getTransactionById", async (transactionId, { rejectWithValue }) => {
  try {
    const endpoint = API_CONFIG.ENDPOINTS.GET_TRANSACTION_BY_ID.replace(
      ":id",
      transactionId
    );
...
>("payment/downloadReceipt", async (transactionId, { rejectWithValue }) => {
  try {
    const endpoint = API_CONFIG.ENDPOINTS.DOWNLOAD_RECEIPT.replace(
      ":id",
      transactionId
    );
    const response = await apiService.get(endpoint);
...
>("payment/getWalletBalance", async (_, { rejectWithValue }) => {
  try {
    const response = await apiService.get(
      API_CONFIG.ENDPOINTS.GET_WALLET_BALANCE
    );
    return response.data.data || response.data;
  } catch (error: any) {
    return rejectWithValue(
...
>("payment/addMoneyToWallet", async (payload, { rejectWithValue }) => {
  try {
    const response = await apiService.post(
      API_CONFIG.ENDPOINTS.ADD_MONEY_TO_WALLET,
      payload
    );
    return response.data.data || response.data;
  } catch (error: any) {
...
>("payment/getWalletTransactions", async (_, { rejectWithValue }) => {
  try {
    const response = await apiService.get(
      API_CONFIG.ENDPOINTS.GET_WALLET_TRANSACTIONS
    );
...
const paymentSlice = createSlice({
  ...
  reducers: {
    ...
    setTransactionFilters: (
      state,
      action: PayloadAction<TransactionFilters>
    ) => {
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(getPaymentMethods.fulfilled, (state, action) => {
        ...
        state.savedCards = action.payload.paymentMethods
          .filter(
            (m): m is PaymentMethod & { details: SavedCard } =>
              m.type === "card"
          )
          .map((m) => m.details);
        state.savedUPI = action.payload.paymentMethods
          .filter(
            (m): m is PaymentMethod & { details: UPIAccount } =>
              m.type === "upi"
          )
          .map((m) => m.details);
        state.defaultPaymentMethod =
          action.payload.paymentMethods.find((m) => m.isDefault) || null;
