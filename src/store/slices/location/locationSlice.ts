...
>("location/getCitiesByState", async (stateId, { rejectWithValue }) => {
  try {
    const endpoint = API_CONFIG.ENDPOINTS.GET_CITIES_BY_STATE.replace(
      ":stateId",
      stateId
    );
    ...
    // Use mock data as fallback for showcase/demo
    console.warn(
      `API unavailable, using mock cities data for state ${stateId}`
    );
    return {
      stateId,
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      return rejectWithValue(
        error.response?.data?.message || "Failed to search garages"
      );
    }
...
>("location/getLocations", async (_, { rejectWithValue }) => {
  try {
    const response = await apiService.get(
      API_CONFIG.ENDPOINTS.GET_ALL_LOCATIONS
    );
...
>("location/getGarageById", async (garageId, { rejectWithValue }) => {
  try {
    const endpoint = API_CONFIG.ENDPOINTS.GET_GARAGE_BY_ID.replace(
      ":id",
      garageId
    );
    const response = await apiService.get(endpoint);
    ...
    if (axios.isAxiosError(error)) {
      return rejectWithValue(
        error.response?.data?.message || "Failed to fetch garage details"
      );
    }
    return rejectWithValue("An unexpected error occurred");
  }
...
>("location/getLocationById", async (locationId, { rejectWithValue }) => {
  try {
    const endpoint = API_CONFIG.ENDPOINTS.GET_LOCATION_BY_ID.replace(
      ":id",
      locationId
    );
    const response = await apiService.get(endpoint);
...
export const selectCitiesByState = (stateId: string) =>
  createSelector(
    [selectLocation],
    (location) => location.citiesByState[stateId] || []
  );
export const selectGarages = createSelector(
  [selectLocation],
  (location) => location.garages
);
...
export const selectSearchLoading = createSelector(
  [selectLocation],
  (location) => location.searchLoading
);
export const selectStatesLoading = createSelector(
  [selectLocation],
  (location) => location.statesLoading
);
export const selectCitiesLoading = createSelector(
  [selectLocation],
  (location) => location.citiesLoading
);
export const selectLocationsLoading = createSelector(
  [selectLocation],
  (location) => location.locationsLoading
);
export const selectLocationError = createSelector(
  [selectLocation],
  (location) => location.error
);
export const selectSelectedGarage = createSelector(
  [selectLocation],
  (location) => location.selectedGarage
);
