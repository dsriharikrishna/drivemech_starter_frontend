...
const PricingCard: React.FC<PricingCardProps> = ({ plan, billingCycle, isSelected, onSelect }) => {
    const price = billingCycle === 'monthly' ? plan.price.monthly : plan.price.yearly;
    const isPopular = plan.isPopular;

    return (
        <div
            className={`relative rounded-2xl p-6 transition-all duration-300 flex flex-col ${isPopular
            ...
            style={isPopular ? { background: 'linear-gradient(to bottom right, #D97642, #B85C2E)' } : undefined}
        >
            {/* Most Popular Badge */}
            {isPopular && (
            ...
            <div className="mb-6">
                ...
                <p className={`text-xs mt-1 ${isPopular ? 'text-orange-100' : 'text-gray-500'}`}>
            ...
            <div className="space-y-5 mb-6 flex-1">
                {plan.features.map((category, idx) => (
                    <div key={idx}>
                        <h4 className={`text-sm font-semibold mb-3 ${isPopular ? 'text-white' : 'text-gray-900'}`}>
                            {category.category}
                        </h4>
                        <ul className="space-y-2">
                            {category.items.map((item, itemIdx) => (
                                <li key={itemIdx} className="flex items-start gap-2">
                                    <Check
                                        className={`w-4 h-4 mt-0.5 flex-shrink-0 ${isPopular ? 'text-white' : 'text-orange-500'
                                            }`}
                                    />
                                    <span className={`text-sm ${isPopular ? 'text-orange-50' : 'text-gray-600'}`}>
                                        {item}
                                    </span>
                                </li>
                            ))}
            ...
            <button
                ...
                className={`w-full py-3 rounded-lg font-semibold transition-all duration-200 ${isPopular
