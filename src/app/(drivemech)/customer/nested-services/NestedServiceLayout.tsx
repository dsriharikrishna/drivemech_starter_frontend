...
} from "@/store/slices/customer/services/serviceSlice";
...
export default function NestedServicesLayout() {
  ...
  const selectedNestedServices = useAppSelector(
    (state) => state.service.selectedNestedServices
  );

  // Reading nested service search query from Redux
  // This is separate from the main service search query
  const searchQuery = useAppSelector(
    (state) => state.service.nestedSearchQuery
  );
  ...
  const nestedServicesAsServiceCards =
    parentService?.nestedServices?.map((ns) => ({
      id: ns.id,
      name: ns.name,
      ...
      badges: [
        {
          icon: "/svgs/nested-services/clock-icon.svg",
          label: "Duration",
          value: "Takes 4 hours",
        },
        {
          icon: "/svgs/nested-services/warranty-icon.svg",
          label: "Warranty",
          value: "500 KMS or 1 Month",
        },
        {
          icon: "/svgs/nested-services/timer-icon.svg",
          label: "Updates",
          value: "Timely Updates",
        },
        {
          icon: "/svgs/nested-services/verified-icon.svg",
          label: "Experts",
  ...
  const filteredNestedServices = nestedServicesAsServiceCards.filter(
    (ns) =>
  ...
  return (
    <FormProvider {...methods}>
      <div className="container mx-auto bg-gray-50 overflow-x-hidden">
