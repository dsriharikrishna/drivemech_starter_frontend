...
import ReadyForDeliveryDetails from "@/components/vendor/ready-for-delivery/ReadyForDeliveryDetails";
...
const ReadyForDeliveryDetailPage = ({ params }: PageProps) => {
  const router = useRouter();
  const { orderId } = use(params);
  const order = getMockOrderData(orderId);

  const handleClose = () => {
    router.push("/vendor/ready-for-delivery");
  };

  return (
    <div className="min-h-screen bg-gray-50">
      {/* Order Details */}
      <ReadyForDeliveryDetails order={order} onClose={handleClose} />
    </div>
  );
};

export default ReadyForDeliveryDetailPage;
