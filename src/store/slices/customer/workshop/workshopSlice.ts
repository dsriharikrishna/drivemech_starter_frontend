...
import {
  ...
  WorkshopBooking,
} from "@/types/customer/workshop/workshopTypes";
...
>("workshop/searchWorkshops", async (searchParams, { rejectWithValue }) => {
  try {
    const response = await apiService.get(
      API_CONFIG.ENDPOINTS.SEARCH_WORKSHOPS,
      {
        params: searchParams,
      }
    );
...
>("workshop/getWorkshopById", async (workshopId, { rejectWithValue }) => {
  try {
    const endpoint = API_CONFIG.ENDPOINTS.GET_WORKSHOP_BY_ID.replace(
      ":id",
      workshopId
    );
...
>("workshop/getNearbyWorkshops", async (params, { rejectWithValue }) => {
  try {
    const response = await apiService.get(
      API_CONFIG.ENDPOINTS.GET_NEARBY_WORKSHOPS,
      {
        params,
      }
    );
    ...
    return rejectWithValue(
      error?.response?.data?.message ||
        error.message ||
        "Failed to fetch nearby workshops"
    );
  }
});
...
>("workshop/getFeaturedWorkshops", async (_, { rejectWithValue }) => {
  try {
    const response = await apiService.get(
      API_CONFIG.ENDPOINTS.GET_FEATURED_WORKSHOPS
    );
    ...
    return rejectWithValue(
      error?.response?.data?.message ||
        error.message ||
        "Failed to fetch featured workshops"
    );
  }
});
...
>("workshop/getWorkshopServices", async (workshopId, { rejectWithValue }) => {
  try {
    const endpoint = API_CONFIG.ENDPOINTS.GET_WORKSHOP_SERVICES.replace(
      ":id",
      workshopId
    );
    const response = await apiService.get(endpoint);
...
>("workshop/getServicePackages", async (workshopId, { rejectWithValue }) => {
  try {
    const endpoint = API_CONFIG.ENDPOINTS.GET_SERVICE_PACKAGES.replace(
      ":id",
      workshopId
    );
    const response = await apiService.get(endpoint);
...
export const getAvailableTimeSlots = createAsyncThunk<
  ...
  { rejectValue: string }
>(
  "workshop/getAvailableTimeSlots",
  async ({ workshopId, date }, { rejectWithValue }) => {
    try {
      const endpoint = API_CONFIG.ENDPOINTS.GET_AVAILABLE_TIME_SLOTS.replace(
        ":id",
        workshopId
      );
...
export const getWorkshopReviews = createAsyncThunk<
  ...
  { rejectValue: string }
>(
  "workshop/getWorkshopReviews",
  async ({ workshopId, page = 1, limit = 10 }, { rejectWithValue }) => {
    try {
      const endpoint = API_CONFIG.ENDPOINTS.GET_WORKSHOP_REVIEWS.replace(
        ":id",
        workshopId
      );
      const response = await apiService.get(endpoint, {
      ...
      return rejectWithValue(
        error?.response?.data?.message ||
          error.message ||
          "Failed to fetch workshop reviews"
      );
    }
  }
);
...
export const createWorkshopBooking = createAsyncThunk<
  ...
  { rejectValue: string }
>(
  "workshop/createWorkshopBooking",
  async (bookingData, { rejectWithValue }) => {
    try {
      const response = await apiService.post(
        API_CONFIG.ENDPOINTS.CREATE_WORKSHOP_BOOKING,
        bookingData
      );
      return response.data.data || response.data;
    } catch (error: any) {
...
const workshopSlice = createSlice({
  ...
  reducers: {
    ...
    updateFilters: (
      state,
      action: PayloadAction<Partial<WorkshopSearchParams>>
    ) => {
