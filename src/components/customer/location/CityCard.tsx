        ...
        >
            ...
            <div className="flex items-center gap-3 mb-3 pb-3 border-b border-gray-200">
                ...
                <div className="flex-1">
                    <h3 className="text-sm font-bold text-gray-900 mb-0.5">{city.name}</h3>
                ...
                <div className="flex-1 max-h-24 overflow-y-auto mb-3">
                    <div className="space-y-1">
                        {city.states.map((state, idx) => (
                            <p key={`${state}-${idx}`} className="text-xs text-gray-600 truncate">
                                • {state}
                            </p>
                        ))}
                    </div>
            ...
            {city.states && city.states.length > 0 && (
                <div className="pt-3 border-t border-gray-200">
                    <p className="text-xs text-gray-500">
                        {city.states.length} {city.states.length === 1 ? 'Region' : 'Regions'}
                    </p>
                </div>
            )}
        </button>
